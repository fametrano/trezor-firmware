...
// Storage upgrade flag and version.
// The first word indicates whether the storage was upgraded before unlock, if
// it was, then a second word follows giving the old storage version.
#define STORAGE_UPGRADE_KEY ((APP_STORAGE << 8) | 0x07)
...
// special handling when both the PIN and wipe code are not set. This value is
// the same as the one used in version 2 storage, which simplifies storage
// upgrade from versions 0 and 1.
static const uint8_t WIPE_CODE_EMPTY[] = {0, 0, 0, 0};
...
static secbool set_wipe_code(const uint8_t *wipe_code, size_t wipe_code_len) {
  if (wipe_code_len == 0) {
    // This is to avoid having to check pin != "" when checking the wipe
    // code.
    wipe_code = WIPE_CODE_EMPTY;
    wipe_code_len = sizeof(WIPE_CODE_EMPTY);
  }
...
static secbool set_pin(const uint8_t *pin, size_t pin_len,
  ...
  if (norcow_active_version != 0 && sectrue != is_not_wipe_code(pin, pin_len)) {
    return secfalse;
  }
...
static void init_wiped_storage(void) {
  ...
  ensure(norcow_set(STORAGE_UPGRADE_KEY, &FALSE_WORD, sizeof(FALSE_WORD)),
         "set_storage_not_upgraded failed");
  ensure(pin_logs_init(0), "init_pin_logs failed");
  ensure(set_wipe_code((const uint8_t *)"", 0), "set_wipe_code failed");

  ui_total = DERIVE_SECS;
  ui_rem = ui_total;
  ui_message = PROCESSING_MSG;
  ensure(set_pin((const uint8_t *)"", 0, NULL), "init_pin failed");
}
...
secbool check_storage_version(void) {
  ...
  const void *storage_upgrade_version = NULL;
  if (sectrue !=
          norcow_get(STORAGE_UPGRADE_KEY, &storage_upgrade_version, &len) ||
  ...
  if (version > norcow_active_version) {
    ...
    if (*(const uint32_t *)storage_upgrade_version != TRUE_WORD) {
      // Attack: The upgrade process was bypassed.
      storage_wipe();
      handle_fault("storage version check");
      return secfalse;
    }
    norcow_set(STORAGE_UPGRADE_KEY, &FALSE_WORD, sizeof(FALSE_WORD));
    ...
    if (*(const uint32_t *)storage_upgrade_version != FALSE_WORD) {
...
// If a storage upgrade is in progress return the (unauthenticated) version we
// are upgrading from, otherwise return current version.
static uint32_t get_upgrade_version(void) {
  const void *val = NULL;
  uint16_t len = 0;
  if (sectrue == norcow_get(STORAGE_UPGRADE_KEY, &val, &len) &&
      len == sizeof(TRUE_WORD) + sizeof(uint32_t)) {
    return ((uint32_t *)val)[1];
  }

  // No upgrade in progress return current version.
  return NORCOW_VERSION;
}
...
secbool storage_change_pin(const uint8_t *oldpin, size_t oldpin_len,
  ...
  secbool ret = set_pin(newpin, newpin_len, new_ext_salt);
  return ret;
}

void storage_ensure_not_wipe_code(const uint8_t *pin, size_t pin_len) {
  // In case of upgrade from version 2 or lower, encode PIN to old format.
  uint32_t legacy_pin = 0;
  if (get_upgrade_version() <= 2) {
    legacy_pin = pin_to_int(pin, pin_len);
...
secbool storage_has_wipe_code(void) {
  ...
  return is_not_wipe_code(WIPE_CODE_EMPTY, sizeof(WIPE_CODE_EMPTY));
}
...
// Legacy conversion of PIN to uint32_t used until storage version 3.
static uint32_t pin_to_int(const uint8_t *pin, size_t pin_len) {
  if (pin_len > V2_MAX_PIN_LEN) {
    return 0;
  }
...
// Legacy conversion of PIN from uint32_t used until storage version 3.
static char *int_to_pin(uint32_t val) {
  static char pin[V2_MAX_PIN_LEN + 1] = {0};
  size_t pos = sizeof(pin) - 1;
  pin[pos] = '\0';
  while (val != 1) {
    if (pos == 0) {
      return NULL;
...
static secbool storage_upgrade(void) {
  ...
  if (norcow_active_version == 0) {
    ...
    // Set the storage version number.
    if (sectrue != storage_set_encrypted(VERSION_KEY, &norcow_active_version,
                                         sizeof(norcow_active_version))) {
      return secfalse;
    }
    ...
    if (sectrue == found && *(const uint32_t *)val != V2_EMPTY_PIN) {
      set_pin((const uint8_t *)val, len, NULL);
    } else {
      set_pin((const uint8_t *)&V2_EMPTY_PIN, sizeof(V2_EMPTY_PIN), NULL);
      ret = norcow_set(PIN_NOT_SET_KEY, &TRUE_BYTE, sizeof(TRUE_BYTE));
    }
  ...
  if (norcow_active_version <= 1) {
    if (sectrue != set_wipe_code((const uint8_t *)"", 0)) {
      return secfalse;
    }
  }
  ...
  if (sectrue != norcow_get(STORAGE_UPGRADE_KEY, &val, &len) ||
      len != sizeof(TRUE_WORD) + sizeof(uint32_t)) {
    uint32_t storage_upgrade_version[2] = {TRUE_WORD, norcow_active_version};
    if (sectrue != norcow_set(STORAGE_UPGRADE_KEY, storage_upgrade_version,
                              sizeof(storage_upgrade_version))) {
      return secfalse;
    }
  }
...
static secbool storage_upgrade_unlocked(const uint8_t *pin, size_t pin_len,
  ...
  if (version == 2) {
    ...
    if (*(uint32_t *)wipe_code_data != 0) {
      char *wipe_code = int_to_pin(*(uint32_t *)wipe_code_data);
      if (wipe_code == NULL) {
        handle_fault("invalid wipe code");
        return secfalse;
      }

      size_t wipe_code_len = strnlen(wipe_code, V2_MAX_PIN_LEN);
